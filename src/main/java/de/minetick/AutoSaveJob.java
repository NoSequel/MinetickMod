...
public class AutoSaveJob {
    ...
    public boolean process() throws ExceptionWorldConflict {
        if(this.isJob(JobDetail.WORLD_SAVE) && this.worldserver != null) {
            MinecraftServer.getLogger().info("[AutoSave] Saving world " + this.worldserver.getWorld().getName());
    ...
    private boolean isFileIOThreadDone() {
        ...
        if(error != null) {
            MinecraftServer.getLogger().error("Incompatibility to FileIOThread in method de.minetick.AutoSaveJob:isFileIOThreadDone()");
            MinecraftServer.getLogger().error(error);
        }
        return done;
    }
    ...
    private void setFileIONoDelay(boolean enabled) {
        ...
        if(error != null) {
            MinecraftServer.getLogger().error("Incompatibility to FileIOThread in method de.minetick.AutoSaveJob:setFileIONoDelay(boolean enabled)");
            MinecraftServer.getLogger().error(error);
        }
    }
}
