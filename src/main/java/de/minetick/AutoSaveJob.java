...
public class AutoSaveJob {
    ...
    public boolean process() throws ExceptionWorldConflict {
        if(this.isJob(JobDetail.WORLD_SAVE) && this.worldserver != null) {
            MinecraftServer.getServer().getLogger().info("[AutoSave] Saving world " + this.worldserver.getWorld().getName());
    ...
    private boolean isFileIOThreadDone() {
        ...
        if(error != null) {
            MinecraftServer.getServer().getLogger().severe("Incompatibility to FileIOThread in method de.minetick.AutoSaveJob:isFileIOThreadDone()");
            MinecraftServer.getServer().getLogger().severe(error);
        }
        return done;
    }
    ...
    private void setFileIONoDelay(boolean enabled) {
        ...
        if(error != null) {
            MinecraftServer.getServer().getLogger().severe("Incompatibility to FileIOThread in method de.minetick.AutoSaveJob:setFileIONoDelay(boolean enabled)");
            MinecraftServer.getServer().getLogger().severe(error);
        }
    }
}
